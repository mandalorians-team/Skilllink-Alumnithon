...
export default function PerfilEstudiante() {
  ...
  const insignias = [
    {
      icon: "🏆",
      title: "Maestro Aprendiz de Front-End",
      desc: "Reconocido por completar con éxito todos los desafíos front-end del semestre, destacando en creatividad y eficiencia.",
      img: "/images/logro1.jpeg",
    },
    {
      icon: "✅",
      title: "Completación de Mentoría",
      desc: "Finalizó 3 programas de mentoría, guiando a compañeros con dedicación y excelentes resultados.",
      img: "/images/logro2.jpeg",
    },
    {
      icon: "🤝",
      title: "Campeón Colaborador",
      desc: "Premiado por su apoyo constante en proyectos grupales y su capacidad de trabajo en equipo.",
      img: "/images/logro3.jpeg",
    },
    {
      icon: "💡",
      title: "Contribuidor Colaborativo",
      desc: "Valorado por aportar ideas innovadoras y soluciones prácticas a retos complejos.",
      img: "/images/logro4.png",
    },
  ];
    ...
    const fetchUsuario = async () => {
      try {
        ...
        if (!usernameFromToken)
          return console.error("No se encontró email en el token");

        const resUsuarios = await fetch("http://localhost:8080/users");
        const users = await resUsuarios.json();
        const usuarioActual = users.find(
          (u) => u.username === usernameFromToken
        );
        if (!usuarioActual)
          return console.error("Usuario no encontrado en lista de usuarios");

        setUsuario(usuarioActual);

        const resDetalles = await fetch(
          `http://localhost:8080/api/learners/${usuarioActual.id}`
        );
        if (resDetalles.ok) {
  ...
  const bio =
    detalles?.bio || "Apasionado por aprender y compartir tecnología.";
  const rol = "Estudiante.";

  return (
    <LayoutEstudiante active="Perfil">
      <div className="flex flex-col min-h-screen bg-[#B8CFDF]">
        ...
        <div className="flex flex-1 mt-16">
          ...
          <main className="flex-1 p-4 space-y-4">
            <h1 className="text-3xl font-bold text-[#19191F] font-orbitron">
              Mi Perfil
            </h1>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <section className="bg-[#19191F] rounded p-4 w-full md:w-1/3">
                <h2 className="text-lg mb-2 text-white font-bold font-orbitron">
                  Detalles Personales
                </h2>
                ...
                {insignias.map((badge, i) => (
                  <div
                    ...
                    <div className="relative w-full h-40 preserve-3d animate-flip-slow hover:pause-spin">
                      <div
                        className="absolute inset-0 flex flex-col justify-center items-center backface-hidden"
                        ...
                        <p className="text-xs text-[#8C8D8B] text-center">
                          {badge.desc}
                        </p>
                      </div>
                      <div
